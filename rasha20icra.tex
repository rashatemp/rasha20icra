...
The contribution of this work is to introduce and compare four active learning strategies that
intelligently pick images taken under new conditions to re-train an existing network:
The first one picks samples based 
on the uncertainty of their predictions, the second one based on their loss with respect 
to pseudo labels. In both cases, we add diversity to the sample selection using a log-space ranking. The third and fourth approach select training samples that are expected to have a maximum effect on the network weights. Even though similar ideas have been explored for active learning in other application contexts, it is non trivial to apply them for semantic segmentation. An important technical novelty in our work is to exploit a pseudo ground truth, which we obtain with very weakly supervised segmentation. Our approach
...
We compare four different approaches to sample selection for active learning. The first one combines uncertainty with a log-based mechanism for achieving diversity (\secref{ssec:unc}). Our main technical contribution is generation of a pseudo ground truth (\secref{ssec:pseudo-ground-truth}) and its use for loss-based (\secref{ssec:loss}), as well as two gradient-based approaches for sample selection (\secref{ssec:normgrad} and \secref{ssec:gradproj}).
...
the images   based on this loss in a descending order and selected on a log-scale ranking to encourage diversity as explained in the previous subsection.
