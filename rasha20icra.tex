...
Our main technical contribution is generation of a pseudo ground truth (\secref{ssec:pseudo-ground-truth}) and its use for loss-based (\secref{ssec:loss}), as well as two gradient-based approaches for sample selection (\secref{ssec:normgrad} and \secref{ssec:gradproj}).
...
where $i$ is the index of the $n$-th sample, $\lvert S \rvert$ is the number of samples to be selected and $\lvert P \rvert$ is the size of the images pool. Since the samples are sorted, this approach would more heavily select those that have higher loss values while not completely discarding images that the network is performing well on.
...
Figures \ref{fig:iou_stuttgart} and \ref{fig:iou_zurich} show the performance on the Stuttgart and Zurich datasets when selecting samples for annotation with different methods. We include as baselines random sampling, and selecting samples that have the highest entropy~(\cite{chakraborty2015active, zhou2017fine}):
\begin{align}
